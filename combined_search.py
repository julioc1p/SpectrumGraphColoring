...
class CombinedSGraphColoring(SpectrumGraphColoring):
    ...
    def _coloring(self, vertex_order, k):
        semi_coloring = self._new_coloring()
        spectrum = self._spectrum[:k]
    ...
    def _swo(self, vertices, k):
        max_iters = 20
        iters = 0
        n_vertices = len(vertices)
        vertex_order = [[v, n_vertices - i] for i, v in enumerate(vertices)]
        solution = None
        best = 1e10
        a = 0.9
        b = 4
        while iters < max_iters:
            coloring = self._coloring(vertex_order, k)
            ...
            iters+=1
        self._tabu_search(solution, self._spectrum[:k])
        return solution
